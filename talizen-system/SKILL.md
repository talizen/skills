...
This is the operating checklist. See `## Reference Routing` for which
`references/*.md` covers what.

## Task Routing

- Treat edits to existing CMS entries as platform-data operations, not source
  edits. Read `references/cms.md`, locate the collection and entry, then update
  the requested fields with the platform CMS tools.
- First confirm whether the field exists and is already rendered. For a
  CMS-value-only edit, skip source versions, patches, and lint; update and
  verify the entry directly.
- Inspect or edit source when the field is missing or the page does not render
  it, such as adding an image `alt` binding.
...
- Never edit `/types/cms.d.ts` or `/types/form.d.ts`. They are generated by the
  platform; update the CMS/Form schema with platform tools, then re-read the
  generated file.
...
- `references/cms.md`: CMS content operations, schema types, and fetch patterns.
...
- A page renders wrong, empty, or not-found while the data exists: re-fetch it
